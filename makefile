...
all:
	...
	$(CC) $(flags) $(IO).o $(ports).o $(kernel).o -T $(kernel).ld -elf32 -o $(kernel).elf -e kernel -nostartfiles 
	mv $(kernel).elf ./iso/boot/
	
	nasm -f bin -g $(loader).s -o $(loader).bin
...
clean:
	rm -f iso/boot/kernel.elf iso/boot/$(loader).bin $(IO).o $(ports).o BOS.iso
